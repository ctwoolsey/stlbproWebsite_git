    ...
    <div class="grouping" id="phones">
    	<?
            ...
            <p class="checkTexting<?=$checkTextingClass?>">
				...
				?>
              <input type="checkbox" name="textingClients" id="textingClients_<?=$phoneCount?>" <?=$checked?>">
                <label for="textingClients_<?=$phoneCount?>">Allow Clients To Text You?</label>
                <br>
                <input type="checkbox" name="textingTeam" id="textingTeam_<?=$phoneCount?>" <?=$checked?>">
                <label for="textingTeam_<?=$phoneCount?>">Allow Texts From stlbridalpros?</label>
                <br>
                <span class="textingCarrierClass<?=$carrierSelectShowClass?>">
                	<!--<label for="textingCarrier_<?=$phoneCount?>">Phone Carrier</label>-->
                    <?
				...
				$smsChecked = "checked";
				$mmsChecked = "";
				if ($phone->textTypeAllowed == MESSAGETYPE::MMS){
					$smsChecked = "";
					$mmsChecked = "checked";
				}
				?>
                <br>
                <span class="textingTypeClass<?=$carrierSelectShowClass?>">
                	Texting Method: 
            	<input type="radio" name="textingType_<?=$phoneCount?>" <?=$mmsChecked?> value="<?=MESSAGETYPE::MMS?>"/>mms
                <input type="radio" name="textingType_<?=$phoneCount?>" <?=$smsChecked?> value="<?=MESSAGETYPE::SMS?>" />sms
                </span>
            </p>
...
function addNewPhone(){
	...
	$(newPhoneBlock).find('.phoneType [type=radio]').each(function(){
	...
	var textingClientsCB = newPhoneBlock.find('[name=textingClients]');
	var newUniqueName = placeUniqueNumberInString(textingClientsCB.prop("id"),phoneCount);
	$(textingClientsCB).prop("id",newUniqueName);
	$(textingClientsCB).parent().find("label").attr("for",newUniqueName);
	var textingTeamCB = newPhoneBlock.find('[name=textingTeam]');
	var newUniqueName = placeUniqueNumberInString(textingTeamCB.prop("id"),phoneCount);
	$(textingTeamCB).prop("id",newUniqueName);
	$(textingTeamCB).parent().find("label").attr("for",newUniqueName);	
	
	var textingCarrierSelect = newPhoneBlock.find('.textingCarrierClass select');
	var newUniqueName = placeUniqueNumberInString(textingCarrierSelect.prop("id"),phoneCount);
	$(textingCarrierSelect).prop("id",newUniqueName);
	$(textingCarrierSelect).prop("name",newUniqueName);
	//$(textingCarrierSelect).parent().find("label").attr("for",newUniqueName);
	
	$(newPhoneBlock).find('.textingTypeClass input').each(function(){
		var newUniqueName = placeUniqueNumberInString($(this).attr("name"),phoneCount);	
		$(this).prop("name",newUniqueName);
	});
	...
	addCanTextEvt(textingTeamCB);
	addCanTextEvt(textingClientsCB);
...
function clearPhone(phoneBlockToClear){
	...
	$(phoneBlockToClear).find('.phoneType [type=radio]').each(function(){
			if ($(this).val() == "cell"){
				$(this).prop("checked",true);
				changePhoneType(this);
			}
		});
	$(phoneBlockToClear).find(".checkTexting").find('[type=checkbox]')
						.each(function(){
							$(this).prop("checked",false);
						});
	$(phoneBlockToClear).find(".textingCarrierClass select").val("-1");
	$(phoneBlockToClear).find(".textingCarrierClass").addClass("hidden");
	$(phoneBlockToClear).find(".textingTypeClass").addClass("hidden");
}

function changePhoneType(phoneTypeRB){
	var phoneBlock = $(phoneTypeRB).parentsUntil(".phoneBlock").parent();
	$(phoneBlock).find(".checkTexting").find('[type=checkbox]')
										   .each(function() {
                                            	$(this).prop("checked",false);
                                        	});
	if ($(phoneTypeRB).val() == "cell"){
		...
		$(phoneBlock).find(".textingCarrierClass").addClass("hidden");
		$(phoneBlock).find(".textingCarrierClass select").val("-1");
		$(phoneBlock).find(".textingTypeClass").addClass("hidden");
	}
}

function addRadioChangeEvent(phoneBlock){
	$(phoneBlock).find('.phoneType [type=radio]').each(function(){
...
function addFrontEndURL(existingURLText){
	var url = existingURLText;
	if (existingURLText != ""){
			existingURLText.replace("https","http");
			if(existingURLText.indexOf("http://www.") == -1){
				if (existingURLText.indexOf("http://") != -1){
					url = existingURLText.replace("http://","http://www.");
				}else{
					url = "http://www." + existingURLText;	
				}
			}
	}
	return url;
...
function getTicketAmount(ticketAmount){
	var amount = "";
	ticketAmount = ticketAmount.replace("$","").trim();
	if (/^\d+(\.\d{2})?$/.test(ticketAmount))
		amount = ticketAmount;
...
function addCanTextEvt(canTextCB){
	$(canTextCB).on("click",function(evt){
		var cbParent = $(this).parent();
		if (!$(cbParent).hasClass("checkTexting"))
			cbParent = $(this).parentsUntil('.checkTexting').parent();
		var textingClientsCB = $(cbParent).find('[name=textingClients]');
		var textingTeamCB = $(cbParent).find('[name=textingTeam]');		   
		
		if (($(textingClientsCB).prop("checked")) || ($(textingTeamCB).prop("checked"))){
			$(".textingCarrierClass").removeClass("hidden");
			$(".textingTypeClass").removeClass("hidden");
		}else{
			$(".textingCarrierClass").addClass("hidden");
			$(".textingTypeClass").addClass("hidden");	
		}
	});
...
function initializeEvents(){
	...
	$('[name=textingClients]').each(function(){
		addCanTextEvt(this);
	});
	$('[name=textingTeam]').each(function(){
		addCanTextEvt(this);
	});
	...
	$("#cancelEditButton").on("click",function(){
		window.location.href="http://www.stlbridalpros.com/vendorAdmin.php"+'<?=$ssidLink_starting?>';
		});
...
function save(closeFormWhenDone){
	var savedVendorObj = {};
			...
			savedVendorObj['facebook'] = addFrontEndURL(getValueNoLabel($("#facebook")));
			savedVendorObj['showFacebook'] = ($("#showFacebook").prop("checked"))?true:false;
			savedVendorObj['instagram'] = addFrontEndURL(getValueNoLabel($("#instagram")));
			savedVendorObj['showInstagram'] = ($("#showInstagram").prop("checked"))?true:false;
			savedVendorObj['twitter'] = addFrontEndURL(getValueNoLabel($("#twitter")));
			savedVendorObj['showTwitter'] = ($("#showTwitter").prop("checked"))?true:false;
			savedVendorObj['pintrist'] = addFrontEndURL(getValueNoLabel($("#pintrist")));
			savedVendorObj['showPintrist'] = ($("#showPintrist").prop("checked"))?true:false;
			savedVendorObj['linkedin'] = addFrontEndURL(getValueNoLabel($("#linkedin")));
			savedVendorObj['showLinkedin'] = ($("#showLinkedin").prop("checked"))?true:false;
			savedVendorObj['googlePlus'] = addFrontEndURL(getValueNoLabel($("#googlePlus")));
			savedVendorObj['showGooglePlus'] = ($("#showGooglePlus").prop("checked"))?true:false;
			savedVendorObj['youtube'] = addFrontEndURL(getValueNoLabel($("#youtube")));
			savedVendorObj['showYoutube'] = ($("#showYoutube").prop("checked"))?true:false;
			savedVendorObj['vimeo'] = addFrontEndURL(getValueNoLabel($("#vimeo")));
			...
			savedVendorObj['ticketToSavingsAmount'] = getTicketAmount(getValueNoLabel($("#ticketToSavingsAmount")));
			...
			$(".phoneBlock").each(function(){
				...
				$(this).find('.phoneType [type=radio]').each(function(){
					if ($(this).prop("checked")){
						phoneObj.type = $(this).val();
					}
				});
				phoneObj.phoneNumber = getValueNoLabel($(this).find(".phoneNumber"));
				if ($(this).find('[name=textingClients]').prop("checked"))
					phoneObj.allowClientsToText = "yes";
				else
					phoneObj.allowClientsToText = "no";
					
				if ($(this).find('[name=textingTeam]').prop("checked"))
					phoneObj.allowTeamToText = "yes";
				else
					phoneObj.allowTeamToText = "no";
					
				var carrierSelectID = $(this).find(".textingCarrierClass").find("select").val();
				if (Number(carrierSelectID) != -1){
					phoneObj.carrierID = carrierSelectID;	
				}
				var textingType = $(this).find(".textingTypeClass")
										 .find('[type=radio]').each(function(){
												if ($(this).prop("checked")){
													phoneObj.textTypeAllowed = $(this).val();
												}
											});
		...
		//public $password = "";
			$.ajax({url:'saveEditedVendor.php', 
				...
				contentType:'application/x-www-form-urlencoded; charset=UTF-8',
				dataType:'json',
				data:{vendorObj:serialize(savedVendorObj),ssid:'<?=$ssid?>'}
			})
			.done(function(success){
				if (success && closeFormWhenDone){
